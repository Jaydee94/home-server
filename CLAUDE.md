...
| Jellyfin  | http://jellyfin.homeserver  | Media-Server; NAS via SMB; LAN/Smart-TV: http://192.168.178.3:8096 (MetalLB); Tailscale Shared Nodes: http://\<tailscale-ip\>:8096 (Klipper) (docs/16-jellyfin.md) |
| Prowlarr  | http://prowlarr.homeserver  | Indexer-Manager (Usenet); Media-Stack (docs/21-media-stack.md) |
| SABnzbd   | http://sabnzbd.homeserver   | Usenet-Downloader; Media-Stack (docs/21-media-stack.md) |
| Sonarr    | http://sonarr.homeserver    | Serien-Automation; Media-Stack (docs/21-media-stack.md) |
| Radarr    | http://radarr.homeserver    | Film-Automation; Media-Stack (docs/21-media-stack.md) |
| Seerr     | http://seerr.homeserver     | Media-Request-UI → Jellyfin; Media-Stack (docs/21-media-stack.md) |
...
| HA-MCP | http://ha-mcp.homeserver/\<Secret-Pfad\> | MCP-Server für Claude Code → Home-Assistant-Dashboards/Automationen; Secret-Pfad nicht im Klartext hier, siehe SealedSecret `ha-mcp-secrets` (docs/22-ha-mcp.md) |
...
- **Jellyfin `jellyfin-tailscale`-Service absichtlich OHNE `loadBalancerClass`**: Der Zweit-Service in `argocd/apps/jellyfin/templates/service-tailscale.yaml` ist das bewusste Gegenstück zum MetalLB-Gotcha — Klipper SOLL ihn übernehmen und hostPort `0.0.0.0:8096` binden (inkl. `tailscale0`), damit Tailscale Shared Nodes Jellyfin direkt über `http://<tailscale-ip>:8096` erreichen (kein Pi-hole-DNS, kein LAN-Routing). Nicht „korrigieren", indem man eine `loadBalancerClass` ergänzt. UFW erlaubt `100.64.0.0/10` bereits vollständig. Details: `docs/16-jellyfin.md`.
...
- **Media-Stack — ein gemeinsamer SMB-Mount + cifs uid/gid 1001**: `argocd/apps/media/` (Sonarr/Radarr/Prowlarr/SABnzbd/Seerr + Exporter) mountet `//jays-ugreen/media` über **ein** statisches SMB-PV (`mediastack-data`) als `/data` in alle Apps. Downloads (`/data/downloads`) UND Bibliothek (`/data/movies`, `/data/shows`) liegen im selben Mount, damit Imports serverseitige **Moves** (Renames) sind statt cross-mount copy+delete. **Hardlinks gehen über SMB/CIFS nicht** — egal beim Usenet-Stack (kein Seeding), *arr auf „move" lassen; echte Hardlinks bräuchten NFS statt SMB. Das PV mountet mit `uid=1001/gid=1001` (linuxserver-PUID/PGID), während das **Jellyfin**-PV denselben Share mit `uid=0` mountet (offizielles Image = root) — zwei separate cifs-Mounts desselben Shares mit unterschiedlichen uid/gid, unkritisch dank `file_mode=0664/dir_mode=0775`. Die *arr-Apps generieren ihre API-Keys selbst (kein seed-Secret); nur SMB-Creds (`mediastack-smbcreds`) sind boot-kritisch. Exporter/Homepage-Keys liegen optional in `media-api-keys`. SABnzbd weist unbekannte Host-Header ab → `sabnzbd.homeserver` in `host_whitelist` eintragen. Details: `docs/21-media-stack.md`.
- **Media-Stack — Sprach-Profile sind Recyclarr-managed (Git, nicht UI)**: Ein Recyclarr-CronJob (`argocd/apps/media/templates/recyclarr.yaml`, täglich 05:30) synct das TRaSH-Guide-German-Profil „UHD Bluray + WEB (GER)" + Custom Formats nach Sonarr/Radarr. Deutsch ist **strikt** erzwungen (`min_format_score: 10000` — englisch-only Releases sind ungrabbar); alle Qualitäten 720p–2160p liegen in EINER gemergten Gruppe, damit der Sprach-Score statt der Auflösung über Upgrades entscheidet. Manuelle UI-Änderungen an diesem Profil werden beim nächsten Sync überschrieben — Anpassungen nur in Git. Der Job braucht `sonarr-api-key`/`radarr-api-key` im SealedSecret `media-api-keys` (sonst `CreateContainerConfigError`). Grabbt nichts mehr → Indexer führt keine deutschen Releases (docs/21-media-stack.md, Abschnitt 9). **Recyclarr — Kein doppelter trash_id in quality_profiles**: Zwei Einträge mit demselben `trash_id` UND unterschiedlichem `qualities:`-Override überspringt Recyclarr stillschweigend die gesamte Service-Sektion. Fix: Zweites Profil als `name:`-only definieren (kein `trash_id`) + `reset_unmatched_scores: enabled: false`, damit per-API gesetzte Scores erhalten bleiben. Nur der DV-Block wird aktiv per `custom_formats.assign_scores_to` verwaltet.
- **HA-MCP — Cross-Namespace-NetworkPolicy-Abhängigkeit**: `argocd/apps/ha-mcp/` (Community-MCP-Server `homeassistant-ai/ha-mcp`, Docker/HTTP-Modus statt der vom Upstream empfohlenen HACS-Installation, da dieses Repo HACS-frei ist) braucht Zugriff auf Home Assistants Port 8123. Die HA-NetworkPolicy (`argocd/apps/home-assistant/templates/networkpolicy.yaml`) erlaubte bisher Ingress nur aus `kube-system` (Traefik) — ohne den zusätzlichen `from:`-Eintrag für den Namespace `ha-mcp` würde der Zugriff stillschweigend blockiert (Timeout statt Fehlermeldung). Auth gegenüber ha-mcp selbst läuft über einen fest gepinnten Secret-URL-Pfad (`MCP_SECRET_PATH`, SealedSecret `ha-mcp-secrets`) statt eines Bearer-Tokens. Der MCP-Client (Claude Code) wird bewusst **lokal/user-scope** registriert, nicht im geteilten `.mcp.json`. Details: `docs/22-ha-mcp.md`.
