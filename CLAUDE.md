...
| Questarr  | http://questarr.homeserver  | Game-Manager (Usenet); Media-Stack (docs/21-media-stack.md) |
...
- **Media-Stack — Sprach-Profile sind Recyclarr-managed (Git, nicht UI)**: Ein Recyclarr-CronJob (`argocd/apps/media/templates/recyclarr.yaml`, täglich 05:30) synct das TRaSH-Guide-German-Profil „UHD Bluray + WEB (GER)" + Custom Formats nach Sonarr/Radarr. Deutsch ist **strikt** erzwungen (`min_format_score: 10000` — englisch-only Releases sind ungrabbar); alle Qualitäten 720p–2160p liegen in EINER gemergten Gruppe, damit der Sprach-Score statt der Auflösung über Upgrades entscheidet. Manuelle UI-Änderungen an diesem Profil werden beim nächsten Sync überschrieben — Anpassungen nur in Git. Der Job braucht `sonarr-api-key`/`radarr-api-key` im SealedSecret `media-api-keys` (sonst `CreateContainerConfigError`). Grabbt nichts mehr → Indexer führt keine deutschen Releases (docs/21-media-stack.md, Abschnitt 9).
- **Media-Stack — Spiele via Questarr (im media-Chart, kein eigener Namespace)**: Für die *arr-Familie gibt es kein Games-Äquivalent — dafür läuft **[Questarr](https://github.com/Doezer/Questarr)** als *arr-artiger Game-Manager **im bestehenden `media`-Chart** (`argocd/apps/media/templates/questarr.yaml`, Namespace `media`), NICHT als eigene `argocd/apps/questarr/`-App. Grund: so teilt es sich das `mediastack-data`-RWX-PV (`/data`) für **atomare Import-Moves** nach `/data/games` und erreicht Prowlarr/SABnzbd per Cluster-DNS (`prowlarr:9696`/`sabnzbd:8080`) — ein Standalone-Namespace bräuchte ein dupliziertes SMB-PV (wie Jellyfin). Questarr ehrt **PUID/PGID** (`1001`, via `media.lsioEnv`-Helper mit `theme ""`, damit KEIN theme.park-DOCKER_MOD gezogen wird — Questarr ist kein lsio-Image); `entrypoint.sh` chownt nur `/app`+`/app/data`, nie den `/data`-Mount (sicher über CIFS). SQLite liegt auf `questarr-config` (local-path, NIE auf SMB). Image `ghcr.io/doezer/questarr` ist als **`v1.4.0`** gepinnt (Tag-Format MIT `v` — Renovate-getrackt via `# renovate:`-Kommentar in `values.yaml → images.questarr`). UI `http://questarr.homeserver` (Ingress via `media.svcIngress`-Helper), Homepage-Kachel *Questarr* in Gruppe *Media*. Indexer/Downloader sind UI-State (Prowlarr-Sync + SABnzbd-Client, Kategorie `games`). **Spiele nicht in Jellyfin abspielbar** (Zugriff per SMB am Share `//jays-ugreen/media/games`). Details: docs/21-media-stack.md, Abschnitt 10.
