...
- **Media-Stack — Spiele laufen ohne *arr (nur SABnzbd-Kategorie)**: Für die *arr-Familie gibt es kein Games-Äquivalent. Spiele werden bewusst minimal über den vorhandenen Usenet-Unterbau geladen — Games-fähiger Newznab-Indexer in **Prowlarr** (getaggt, **nicht** an Sonarr/Radarr gesynct), manueller Grab, SABnzbd-Kategorie `games` mit Zielordner `/data/games` (absoluter Pfad, überschreibt `/data/downloads/complete/…`). Kein neues Deployment, kein Chart-Change — SABnzbd-Kategorie + Prowlarr-Indexer sind UI-State auf den Config-PVCs. **Spiele sind nicht in Jellyfin abspielbar** (Zugriff per SMB am NAS-Share `//jays-ugreen/media/games`). Homepage verlinkt sie als Kachel *Games* → SABnzbd. Cleanup-Liste für `games` entschärfen (Games-`.nfo` nicht löschen); bei großen Spielen `incomplete.size` erhöhen (SSD-Unpack). Details: docs/21-media-stack.md, Abschnitt 10.
