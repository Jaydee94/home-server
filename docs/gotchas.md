...
- **Namespace-Isolation seit `docs/23-network-policies.md`**: Ein Teil der App-Namespaces (u. a. `media`, `minio`, `monitoring`, `argo-workflows`, `homepage`) erlaubt Ingress nur noch aus dem eigenen Namespace + Traefik (`kube-system`) + explizit gelisteten Cross-Namespace-Callern. Eine **neue** App, die per `<svc>.<namespace>.svc.cluster.local` in einen dieser Namespaces hineinruft, bricht sonst **stillschweigend** (Connection Timeout, keine Fehlermeldung) — die `networkpolicy.yaml` der Ziel-App muss um einen `namespaceSelector`-Eintrag für den neuen Caller ergänzt werden. Vollständige Liste der abgedeckten/bewusst ausgenommenen Namespaces: `docs/23-network-policies.md`.
- **Bewusst NICHT abgesichert**: `pihole`, `mosquitto`, `jellyfin` (dedizierte MetalLB-IPs — Traffic kommt nicht mit einer per `podSelector`/`namespaceSelector` matchbaren Quelle an), `metallb`, `sealed-secrets`, `kubevirt`, `gameserver`, `csi-driver-smb` (Webhook-/Controller-/Gossip-Kommunikation, hohes Risiko einer falsch geschnittenen Policy). Nicht "nachbessern" ohne Testfenster — siehe Begründung in `docs/23-network-policies.md`.

## Secrets / SealedSecrets — Admin-Passwörter

- **Neue Apps mit Admin-UI starten oft bewusst mit leerem/deaktiviertem Passwort**: Damit ein frischer ArgoCD-Sync ohne manuellen Zwischenschritt `Healthy` wird, liefern mehrere Charts (Pi-hole, Grafana, MinIO, Mosquitto, Gotify) den Admin-Zugang standardmäßig ohne Secret aus (`adminPassword: ""` + `adminSecret.enabled: false` bzw. äquivalent) — die Admin-UI ist dann bis zum ersten Sealing **unauthentifiziert** erreichbar (nur LAN/Tailnet-geschützt). **Direkt nach dem ersten erfolgreichen Sync eines solchen App-Verzeichnisses**: Passwort sealen (`kubeseal --raw --namespace <ns> --name <secret-name> --controller-name sealed-secrets-controller --controller-namespace sealed-secrets --from-file=/dev/stdin`), `encryptedPassword` in `values.yaml` eintragen, `adminSecret.enabled: true` **und** die passende `existingSecret:`-Referenz im Chart-Values-Block setzen — beide Stellen sind nötig, ein Sealing allein reicht nicht. Muster/Doku-Kommentare dazu stehen direkt in den jeweiligen `values.yaml` (z. B. `argocd/apps/pihole/values.yaml`).
- **Aktueller Stand in diesem Repo**: Pi-hole, Grafana, MinIO, Mosquitto und Gotify haben bereits reale gesealte Passwörter hinterlegt (`encryptedPassword` ist gesetzt, `existingSecret` verweist darauf) — das leere-Default-Muster ist hier nur beim erstmaligen App-Bootstrap relevant, nicht im laufenden Betrieb. Bei einem neuen Fork/Clone oder beim Hinzufügen einer neuen App mit Admin-UI diesen Schritt nicht vergessen.
